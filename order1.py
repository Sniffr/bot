...
def add_order_items_to_cart(driver):
    ...
    number_to_add = random.randint(5, 15)
    ...
    for button in buttons:
        ...
        quantity_input.send_keys(random.randint(10, 30))
...
def run_order_driver(username, password):
    ...
    try:
        logging.info(f"Running order driver for {username} at {datetime.datetime.now().strftime('%H:%M:%S')}")
        ...
        logging.error(f"Error running driver for {username}: {e}")
    finally:
        # Ensure driver quits even if there was an error
        driver.quit()
...
def schedule_orders(users_list):
    ...
    accounts_once = random.sample(list(users_list), random.randint(18, 30))
    ...
    for account in accounts:
        ...
        for _ in range(num_orders):
            ...
            orders_collection.insert_one({
                ...
                "date": date_time.now().strftime("%Y-%m-%d"),
                "completed": False
            })
...
def random_time_within_business_hours():
    ...
    return date_time.now().replace(hour=hour, minute=minute, second=0, microsecond=0)


def run_task(task):
    print(f"Executing task for {task['username']} at {date_time.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def schedule_all_tasks():
    tasks = orders_collection.find({"completed": False, "date": date_time.now().strftime("%Y-%m-%d")})
    for task in tasks:
        schedule_task(task)
...
def daily_schedule():
    logging.info("Checking if a schedule is made for today...")
    if not is_schedule_made_for_today():
        logging.info("No schedule made for today. Making schedule...")
        # Your logic to fetch users and schedule orders
        schedule_orders(list(users))
    else:
        logging.info("Schedule already made for today.")

    logging.info("Scheduling all tasks for today...")
    schedule_all_tasks()
