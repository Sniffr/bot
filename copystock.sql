INSERT INTO stock (alert_quantity, branch_id, cost_price, created_by, created_date, desired_quantity,
...
FROM users AS u1
         ...
         INNER JOIN jungo.branch AS b2 ON b.id = b2.business_id
         INNER JOIN stock AS s ON s.branch_id = b2.id
         INNER JOIN jungo.business AS b_new ON b_new.owner_id = new_owner_id

         INNER JOIN jungo.branch AS b2_new ON b_new.id = b2_new.business_id
WHERE u1.id = 1536;

select * from stock
    inner join  jungo.branch b on stock.branch_id = b.id
    inner join business on b.business_id = business.id
where business_id = 1273;

UPDATE stock
    INNER JOIN jungo.branch b ON stock.branch_id = b.id
    INNER JOIN business ON b.business_id = business.id
SET stock.quantity = 5000